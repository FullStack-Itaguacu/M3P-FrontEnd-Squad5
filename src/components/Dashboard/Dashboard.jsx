    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3333/api/sales/dashboard/admin`,
          {
            headers: {
              Authorization: `${token}`,
            },
        ...
        const { totalValue, totalAmount } = response.data;
        setSalesData({ totalValue, totalAmount });
      } catch (error) {
        console.error("Error fetching data:", error);
        setSalesData({ totalValue: 0, totalAmount: 0 });
      }
    };
  ...
  return (
    <div className={`pb-5 ${styles.dashboardStyle}`}>
      ...
      <p className="text-center text-black">
        Abaixo você pode acompanhar um resumo dos seus resultados até agora:
      </p>
      <Stack
        ...
        <Col md={2}>
          <Card className={`p-3 ${styles.cardStyle}`}>
            <Card.Body>
              ...
              <Card.Title className="text-center text-black fs-3">
                Total de Vendas (R$)
              </Card.Title>
              <Card.Text className="text-center text-black fs-1 fw-bold">
